#!/bin/bash
set -o errexit
openssl req -x509 -new -newkey rsa:2048 -out root.pem -keyout root-key.pem -days 3650 -nodes -subj "/CN=happy hacker fake ROOT 2" -extensions v3_ca -config root.cnf

openssl x509 -text -noout -in root.pem

openssl req -new -newkey rsa:2048 -out cross.csr -keyout cross-key.pem -days 1825  -nodes -subj "/CN=happy hacker fake INTERMEDIATE 2" -extensions v3_ca -config cross.cnf
